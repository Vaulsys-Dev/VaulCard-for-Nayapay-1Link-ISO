        ...
        try {
            ...
            */
            ...
            try {
                ...
                GeneralDao.Instance.flush(); // Asim Shahzad, Date : 7th Sep 2022, Tracking ID : VC-NAP-202207262
...
//            account.setAvailableBalance((AcctAvailBalance + (DepositAmount - (NayaPayChargeAmount + BankCharges + BankTaxAmount + SrcChargeAmount))) + "");
    ...
    public static boolean BalanceInquiry(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            ...
            */
            ...
            //m.rehman: 21-02-2022, VC-NAP-202202211 - Exception occurred on VaulCard while executing 1Link Cash Withdrawal transaction
            //adding below check to handle database exception
            boolean isCommitted;
            try {
                //GeneralDao.Instance.endTransaction();
        ...
        catch (Exception e)
        {
            ...
            //m.rehman: 21-06-2023, VP-NAP-202306202 - Processing Error while Executing IBFTout - saad.069
            try {
                logger.error("Rolling back transaction ...");
                GeneralDao.Instance.rollback();
            } catch (HibernateException ee) {
                logger.error("Error occurred while rolling back transaction, rejecting ...");
                wsmodel.setRespcode(ISOResponseCodes.INTERNAL_DATABASE_ERROR);
            }
            finally {
                GeneralDao.Instance.close();
    ...
    @Transactional
    public static boolean MerchantBalanceInquiry(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            logger.info("UnLoading Wallet for MerchantBalanceInquiry Transaction....");
            ...
            */
            ...
            if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("PayPak")) {
                // Asim Shahzad, Date : 25th Sep 2020, Tracking ID : VP-NAP-202009081/VC-NAP-202009081
                UpdateandLogArc1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, true, true, true, false);
                // ===================================================================================
            } else if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("Visa")) {
                UpdateandLogArc1LinkVisaSettlementtAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, true, true, true);
            }
            ...
            */
            ...
            GeneralDao.Instance.beginTransaction(GeneralDao.OPTIMIZER_MODE_FIRST_ROWS);
            if (isCommitted) {
                logger.info("MerchantBalanceInquiry in Wallet completed successfully!");
                return true;
            } else {
                wsmodel.setRespcode(ISOResponseCodes.INTERNAL_DATABASE_ERROR);
                return false;
    ...
    @Transactional
    public static boolean MerchantIBFTIn(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            logger.info("Loading Wallet for Merchant IBFTIn Transaction....");
            ...
            */
            ...
            //UpdateandLogCollectionAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, TxnFlag.CASH_IN, AccType.CAT_SETT_ACCT);
            // Asim Shahzad, Date : 25th Sep 2020, Tracking ID : VP-NAP-202009081/VC-NAP-202009081
            UpdateandLogArc1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.DEBIT, true, false, true, false);
            // ===================================================================================

            if(SrcChargeAmount > 0) //TODO: Raza use with above Check
            {
                UpdateandLogArcRevenueAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT);
            }

            if(Tax > 0)
            {
                UpdateandLogArcTaxAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT);
...
//            } // Asim Shahzad, Date : 19th Oct 2020, Desc : commented this check to rectify issue in settlement, Tracking ID : VP-NAP-202010051
    ...
    public static boolean FundManagement(WalletCMSWsEntity wsmodel)
    {
        try {
            ...
            fundBankAccount = (CMSEMIAccountCollection)GeneralDao.Instance.getCurrentSession().get(CMSEMIAccountCollection.class, fundBankAccount.getId(), LockMode.UPGRADE);
...
//                    fundBankAccount.setAvailableBalance(StringUtils.leftPad(acctAvailUpdatedBalance.toString(), 12, "0"));
    ...
    public static boolean IBFT(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            ...
            // Asim Shahzad, Date : 25th Sep 2020, Tracking ID : VP-NAP-202009081/VC-NAP-202009081
            //Waleed making isChargesApply false due to IBFT Charges implementation
            UpdateandLog1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, false, false, true, false);
            // ===================================================================================
    ...
    @Transactional
    public static boolean MerchantIBFT(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            logger.info("Unloading Wallet for MerchantIBFT Transaction....");
            ...
            //Waleed making isChargesApply false due to IBFT Charges implementation
            //UpdateandLog1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, false, false, true, false);
            UpdateandLogArc1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, false, false, true, false);
            // ===================================================================================
            ...
            GeneralDao.Instance.beginTransaction(GeneralDao.OPTIMIZER_MODE_FIRST_ROWS);
            if (isCommitted) {
                logger.info("MerchantIBFT in Wallet completed successfully!");
                return true;
            } else {
                wsmodel.setRespcode(ISOResponseCodes.INTERNAL_DATABASE_ERROR);
                return false;
        ...
        {
            logger.error("Exception caught while MerchantIBFT in wallet...");
            e.printStackTrace();
            wsmodel.setRespcode(ISOResponseCodes.UNABLE_TO_PROCESS); //46 - Unable to Process ; Refer to Doc
...
//                balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccountpayable.getAvailableBalance(), 12, "0"));
    ...
    {
        CMSEMIWallet reveneuewallet = GetRevenueWallet(currency,wsmodel);
...
//        collbalanceLog.setClosingBalance(StringUtils.leftPad(revenueAccount.getAvailableBalance(), 12, "0"));
    ...
    {
        CMSEMIWallet taxwallet = GetSalesTaxWallet(currency,wsmodel);
...
//        collbalanceLog.setClosingBalance(StringUtils.leftPad(liabilityAccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static void UpdateandLogArcTaxAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag)
    {
        CMSEMIWallet taxwallet = GetSalesTaxWallet(currency,wsmodel);
        CMSEMIAccountCollection liabilityAccount = GetEMICollectionAccount(currency, AccType.CAT_ARC_LIABILITY_ACCT);
...
//        collbalanceLog.setClosingBalance(StringUtils.leftPad(liabilityAccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static void UpdateandLogArcRevenueAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag)
    {
        CMSEMIWallet reveneuewallet = GetRevenueWallet(currency,wsmodel);
        CMSEMIAccountCollection revenueAccount = GetEMICollectionAccount(currency, AccType.CAT_ARC_REVENUE_ACCT);
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static CMSEMIWallet GetRevenueWallet(String currency,WalletCMSWsEntity wsmodel)
    {
        String dbQuery;
...
//        params.put("CURR", currency);
        if(wsmodel.getIsmerchanttransaction() != null && wsmodel.getIsmerchanttransaction().equals(true)){
            params.put("CAT", AccType.CAT_ARC_REVENUE_WLLT.StringValue());
        }else{
    ...
    @Transactional
    public static CMSEMIWallet GetSalesTaxWallet(String currency,WalletCMSWsEntity wsmodel)
    {
        String dbQuery;
        Map<String, Object> params;
...
//        params.put("CURR", currency);
        if(wsmodel.getIsmerchanttransaction() != null && wsmodel.getIsmerchanttransaction().equals(true)){
            params.put("CAT", AccType.CAT_ARC_SALESTAX_WLLT.StringValue());
        }else{
...
//        params.put("CURR", currency);
    ...
    public static CMSBusinessWallet GetBusinessWallet(String currency, String category)
    {
        ...
        businessWallet = (CMSBusinessWallet)GeneralDao.Instance.getCurrentSession().get(CMSBusinessWallet.class, businessWallet.getId(), LockMode.UPGRADE);
    ...
    public static CMSEMIWallet GetThunesSettLedgerAccount(String currency)
    {
        ...
        CMSEMIWallet settlementaccount = (CMSEMIWallet)GeneralDao.Instance.findObject(dbQuery, params);
        settlementaccount = (CMSEMIWallet)GeneralDao.Instance.getCurrentSession().get(CMSEMIWallet.class, settlementaccount.getId(), LockMode.UPGRADE);
...
//        params.put("CURR", currency);
        ...
        billeraccount = (CMSEMIAccountCollection)GeneralDao.Instance.getCurrentSession().get(CMSEMIAccountCollection.class, billeraccount.getId(), LockMode.UPGRADE);
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(suspenseAccount.getAvailableBalance(), 12, "0"));
    ...
    //Added By Waleed for Merchant IBFTIN - SUSPENSE PARKING
    @Transactional
    public static void UpdateandLogArcSuspenseAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag, boolean isChargesApply, boolean isApplyOnlyCharges, boolean isChargesAdd)
    {
        CMSEMIAccountCollection suspenseAccount = GetEMICollectionAccount(currency, AccType.CAT_ARC_SUSPENSE_ACCT);
        Long AcctActBalance = (Util.hasText(suspenseAccount.getActualBalance()) ? Long.parseLong(suspenseAccount.getActualBalance()) : 0L);
        Long AcctAvailBalance = (Util.hasText(suspenseAccount.getAvailableBalance()) ? Long.parseLong(suspenseAccount.getAvailableBalance()) : 0L);

        CMSEMIWallet suspenseWallet = GetEMIWallet(currency, AccType.CAT_ARC_SUSPENSE_WLLT);
        Long WlltActBalance = (Util.hasText(suspenseWallet.getActualBalance()) ? Long.parseLong(suspenseWallet.getActualBalance()) : 0L);
        Long WlltAvailBalance = (Util.hasText(suspenseWallet.getAvailableBalance()) ? Long.parseLong(suspenseWallet.getAvailableBalance()) : 0L);
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccountpayable.getAvailableBalance(), 12, "0"));
    ...
    //added by waleed
    @Transactional
    public static void UpdateandLogArc1LinkReceivableAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag,
                                                          boolean isChargesApply, boolean isApplyOnlyCharges, boolean isChargesAdd) {

        // Asim Shahzad, Date : 19th May 2021, Tracking ID : VC-NAP-202105061
        CMSEMIAccountCollection settlementaccountreceiveable = GetEMICollectionAccount(currency, AccType.CAT_ARC_1LINK_SETT_RECEIVABLE_ACCT);
        Long AcctActBalance = (Util.hasText(settlementaccountreceiveable.getActualBalance()) ? Long.parseLong(settlementaccountreceiveable.getActualBalance()) : 0L);
        Long AcctAvailBalance = (Util.hasText(settlementaccountreceiveable.getAvailableBalance()) ? Long.parseLong(settlementaccountreceiveable.getAvailableBalance()) : 0L);

        CMSEMIWallet settlementWalletReceiveable = GetEMIWallet(currency, AccType.CAT_ARC_1LINK_SETT_RECEIVABLE_WLLT);
        Long WlltActBalance = (Util.hasText(settlementWalletReceiveable.getActualBalance()) ? Long.parseLong(settlementWalletReceiveable.getActualBalance()) : 0L);
        Long WlltAvailBalance = (Util.hasText(settlementWalletReceiveable.getAvailableBalance()) ? Long.parseLong(settlementWalletReceiveable.getAvailableBalance()) : 0L);
...
//            balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    //m.rehman: 20-04-2022, VC-NAP-202205092 -  IBFTIn Transaction not handled in FMS 08th May,2022- Production
    public static boolean IBFTInFromSP(WalletCMSWsEntity wsmodel)
    {
        CallableStatement callableStatement = null;
        try {
            logger.info("Loading Wallet for IBFTIn Transaction....");

            String queryStr = "{ CALL PACKAGE_TRANSACTION.spIBFTIn(?,?,?) }";
            ...
            logger.info("IBFTIn, executing SP");
            callableStatement.execute();
            String respCode = callableStatement.getString(2);
            Integer spResponse = callableStatement.getInt(3);
        ...
        {
            logger.error("Exception caught while IBFTIn in wallet...");
            logger.error(WebServiceUtil.getStrException(e));
            wsmodel.setRespcode(ISOResponseCodes.UNABLE_TO_PROCESS); //46 - Unable to Process ; Refer to Doc
            return false;
        }
    ...
    @Transactional
    public static boolean MerchantCashWithDrawal(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            logger.info("UnLoading Wallet for CashWithDrawal Transaction....");
            ...
            if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("PayPak")) {
                ...
                //m.rehman: 15-07-2021, VC-NAP-202107151 - Total amount not crediting in Visa Local Cash Withdrawal/ECommerce reversal amount not crediting in wallet
                //setting isApplyOnlyCharges parameter from true to false
                UpdateandLogArc1LinkVisaSettlementtAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, true, false, true);
            }
    ...
    @Transactional
    public static void UpdateandLogArc1LinkVisaSettlementtAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag,
                                                               boolean isChargesApply, boolean isApplyOnlyCharges, boolean isChargesAdd)
    {

        CMSEMIAccountCollection settlementaccount = GetEMICollectionAccount(currency, AccType.CAT_ARC_1LINK_VISA_SETT_PAYABLE_ACCT);
        CMSEMIWallet settlementWallet = GetEMIWallet(currency, AccType.CAT_ARC_1LINK_VISA_SETT_PAYABLE_WLLT);
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static boolean MerchantCardBasedReversal(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            logger.info("Loading Wallet for " + wsmodel.getServicename() + " Transaction....");
            ...
            */
            ...
            if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("PayPak")) {
                // Asim Shahzad, Date : 25th Sep 2020, Tracking ID : VP-NAP-202009081/VC-NAP-202009081
                UpdateandLogArc1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.DEBIT, true, false, true, true);
                // ===================================================================================
            } else if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("Visa")) {
                if (wsmodel.getOriginalapi().equals("Purchase") || wsmodel.getOriginalapi().equals("ECommerce")
                        || wsmodel.getOriginalapi().equals("MOTO")) {
                    UpdateandLogArcVisaLocalSettlementtAccount(wsmodel, account.getCurrency(), TxnFlag.DEBIT, true, false, true);
                }
                else {
                    UpdateandLogArc1LinkVisaSettlementtAccount(wsmodel, account.getCurrency(), TxnFlag.DEBIT, true, false, true);
                }
            }
    ...
    @Transactional
    public static void UpdateandLogArcMerchantVisaLocalSettlementtAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag,
                                                               boolean isChargesApply, boolean isApplyOnlyCharges, boolean isChargesAdd)
    {

        CMSEMIAccountCollection settlementaccount = GetEMICollectionAccount(currency, AccType.CAT_ARC_VISA_LCL_SETT_PAYABLE_ACCT);
        CMSEMIWallet settlementWallet = GetEMIWallet(currency, AccType.CAT_ARC_VISA_LCL_SETT_PAYABLE_WLLT);
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static boolean MerchantPurchase(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try {
            logger.info("UnLoading Wallet for Purchase Transaction....");
            ...
            */
            ...
            if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("PayPak")) {
                // Asim Shahzad, Date : 25th Sep 2020, Tracking ID : VP-NAP-202009081/VC-NAP-202009081
                UpdateandLogArc1LinkAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, true, false, true, false);
                // ===================================================================================
            } else if (Util.hasText(wsmodel.getCardscheme()) && wsmodel.getCardscheme().equals("Visa")) {
                UpdateandLogArcVisaLocalSettlementtAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT, true, true, true);
            }
    ...
    @Transactional
    public static void UpdateandLogArcVisaLocalSettlementtAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag,
                                                               boolean isChargesApply, boolean isApplyOnlyCharges, boolean isChargesAdd)
    {
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static void UpdateandLogArc1LinkAccount(WalletCMSWsEntity wsmodel, String currency, String txnFlag,
                                                boolean isChargesApply, boolean isApplyOnlyCharges, boolean isChargesAdd, boolean isReversalFlag)
    {
        ...
        if(!isReversalFlag) {
            if (txnFlag.equals(TxnFlag.DEBIT)) {
                ...
                CMSEMIAccountCollection settlementaccountreceivable = GetEMICollectionAccount(currency, AccType.CAT_ARC_1LINK_SETT_RECEIVABLE_ACCT);
                Long AcctActBalance = (Util.hasText(settlementaccountreceivable.getActualBalance()) ? Long.parseLong(settlementaccountreceivable.getActualBalance()) : 0L);
                Long AcctAvailBalance = (Util.hasText(settlementaccountreceivable.getAvailableBalance()) ? Long.parseLong(settlementaccountreceivable.getAvailableBalance()) : 0L);

                CMSEMIWallet settlementWalletReceivable = GetEMIWallet(currency, AccType.CAT_ARC_1LINK_SETT_RECEIVABLE_WLLT);
                Long WlltActBalance = (Util.hasText(settlementWalletReceivable.getActualBalance()) ? Long.parseLong(settlementWalletReceivable.getActualBalance()) : 0L);
                Long WlltAvailBalance = (Util.hasText(settlementWalletReceivable.getAvailableBalance()) ? Long.parseLong(settlementWalletReceivable.getAvailableBalance()) : 0L);
...
//                balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccountreceivable.getAvailableBalance(), 12, "0"));
                ...
                CMSEMIAccountCollection settlementaccountpayable = GetEMICollectionAccount(currency, AccType.CAT_ARC_1LINK_SETT_PAYABLE_ACCT);
                Long AcctActBalance = (Util.hasText(settlementaccountpayable.getActualBalance()) ? Long.parseLong(settlementaccountpayable.getActualBalance()) : 0L);
                Long AcctAvailBalance = (Util.hasText(settlementaccountpayable.getAvailableBalance()) ? Long.parseLong(settlementaccountpayable.getAvailableBalance()) : 0L);

                CMSEMIWallet settlementWalletPayable = GetEMIWallet(currency, AccType.CAT_ARC_1LINK_SETT_PAYABLE_WLLT);
                Long WlltActBalance = (Util.hasText(settlementWalletPayable.getActualBalance()) ? Long.parseLong(settlementWalletPayable.getActualBalance()) : 0L);
                Long WlltAvailBalance = (Util.hasText(settlementWalletPayable.getAvailableBalance()) ? Long.parseLong(settlementWalletPayable.getAvailableBalance()) : 0L);
