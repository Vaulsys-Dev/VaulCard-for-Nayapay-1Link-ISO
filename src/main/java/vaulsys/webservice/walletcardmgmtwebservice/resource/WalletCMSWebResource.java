...
public class WalletCMSWebResource {
	...
	//=======================================================================================================
	// Asim Shahzad, Date : 24th Aug 2023, Tracking ID : VP-NAP-202307311
	@Path("/getnotificationdetails")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public WalletCMSWsEntity GetNotificationDetails(WalletCMSWsEntity wsobj) throws Exception{
		...
		try {
			...
			logger.info("WebResource GetNotificationDetails Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setIncomingip(request.getRemoteAddr());
			return WSOperation.ExecuteGetNotificationDetails(wsobj);
		}
		catch (Exception e)
		{
			e.printStackTrace();
			logger.error("Exception caught while executing GetNotificationDetails call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending GetNotificationDetails reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		finally {
	...
	// Asim Shahzad, Date : 20th Nov 2023, Tracking ID : VC-NAP-202303062
	@Path("/getfinancialdeductiondetails")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public WalletCMSWsEntity GetFinancialDeductionDetails(WalletCMSWsEntity wsobj) throws Exception{
		...
		try {
			...
			logger.info("WebResource GetFinancialDeductionDetails Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setIncomingip(request.getRemoteAddr());
			return WSOperation.ExecuteGetFinancialDeductionDetails(wsobj);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	@Path("/cashwithdrawalinquiry")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	//add MediaType.APPLICATION_XML if you want XML as well (don't forget @XmlRootElement)
	public WalletCMSWsEntity CashWithDrawalInquiry(WalletCMSWsEntity wsobj) throws Exception {

		try {
			logger.info("WebResource CashWithDrawalInquiry Request Received");
			if (wsobj == null) {
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
				wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			...
			wsobj.setServicename("CashWithDrawalInquiry");
			//m.rehman: for validating incoming ip address
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
			...
			e.printStackTrace();
			logger.error("Exception caught while executing CashWithDrawalInquiry call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending CashWithDrawalInquiry reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		finally {
			HttpSession session = request.getSession(false);
