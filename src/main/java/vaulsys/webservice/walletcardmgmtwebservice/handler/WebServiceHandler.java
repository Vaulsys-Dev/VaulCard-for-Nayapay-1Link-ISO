...
import vaulsys.webservice.walletcardmgmtwebservice.entity.IBFTInTracker;
...
//    }
    ...
    public static WalletCMSWsEntity processIBFTIn(WalletCMSWsEntity wsmodel)
    {
        ...
        {
            try {
                IBFTInTracker ibftInTracker = new IBFTInTracker();
                ibftInTracker.setServicename(wsmodel.getServicename());
                ibftInTracker.setStan(wsmodel.getStan());
                ibftInTracker.setRrn(wsmodel.getRrn());
                /*if(wsmodel.getIsmerchanttransaction() != null && wsmodel.getIsmerchanttransaction().equals(true)) {
                    ibftInTracker.setUserid(wsmodel.getMerchantid());
                }
                else{
                    ibftInTracker.setUserid(wsmodel.getUserid());
                }*/
                ibftInTracker.setTransdatetime(wsmodel.getTransdatetime().substring(4,14));
                GeneralDao.Instance.save(ibftInTracker);
                GeneralDao.Instance.flush();  // Ensure changes are sent to the database
            } catch (Exception e){
                logger.error("Exception caught while saving IBFTInTracker [" + wsmodel.getServicename() + "] transaction, rejecting...");
                if (e instanceof ConstraintViolationException) {
                    logger.error("Duplicate Transaction Exception for servicename [" + wsmodel.getServicename() +
                            "], Tranrefnumber [" + wsmodel.getTranrefnumber() + "]...!");
                    wsmodel.setRespcode(ISOResponseCodes.DUPLICATE_TRANSACTION); //Duplicate Transaction Refer to Document //For same txnrefnum and transdatetime
                } else {
                    logger.error(WebServiceUtil.getStrException(e));
                    wsmodel.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
                }
    ...
    public static WalletCMSWsEntity processCashWithDrawalInquiry(WalletCMSWsEntity wsmodel)
    {
        return WSOperation.ExecuteCashWithDrawalInquiry(wsmodel);
    }
