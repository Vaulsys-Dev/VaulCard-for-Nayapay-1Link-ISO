...
public class HSMNetworkManager implements Manager {
    ...
    //m.rehman: 28-12-2021, Nayapay Optimization
    private static String prevMesg;
    ...
    public void receiveResponseFromHSM(byte[] msg, HSMChannel hsmChannel) throws InterruptedException, NotAvailableHSMChannelFoundException,
        ...
        if (hsmChannel != null && Util.hasText(hsmChannel.getCommandTypeDesc())
            ...
            */
            ...
            //split message with respect to end character #> as multiple response receiving
            String[] respArray = hsmResponse.split("\\r?\\n");
            for (String response : respArray) {
                ...
                if (response.startsWith("<") && response.endsWith("#>")) {
                    ...
                    if (response.contains("^")) {
                        ...
                        index += 1;
                        String sequence = response.substring(index, response.indexOf("#", index));
                        ...
                        for (Thread thread : threads) {
                            if (thread.getName().contains(threadName)) {
                                origTxn = thread;
                                //GlobalContext.getInstance().setHsmResponse(thread.getName(), msg);
                                GlobalContext.getInstance().setHsmResponse(thread.getName(), response.getBytes());
                                break;
                            }
                        }
                    ...
                    logger.info("Incomplete response received [" + response + "], saving in buffer and wait for next message ...");
                    prevMesg = response;

                } else if (!response.startsWith("<") && response.endsWith("#>") && Util.hasText(prevMesg)) {
                    logger.info("Remaining message received [" + response + "] of earlier response message, processing ...");

                    response = prevMesg + response;
                    prevMesg = "";
                    logger.info("Complete response [" + response + "] ...");
